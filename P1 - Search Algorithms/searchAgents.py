...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # a tuple which holds the starting position and a list for the 4 corners
        starting_tuple = (self.startingPosition, [])
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        position, four_corners = state
        if position in four_corners:
            if len(self.corners) == len(four_corners):
                return True
        return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            x, y = state[0]
            four_corners = state[1]
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            next_visited_corners = list(four_corners)
            ...
            if not hitsWall:
                ...
                next_state = (next_pos, next_visited_corners)
                cost = 1
